...
int main(int argc, char *argv[]) {
  ...
  #pragma omp parallel for schedule(dynamic) shared(spring) num_threads(2)
  for(i=0; i<600; ++i) {
...
void springRungeKutta4(struct SpringODE *spring, double ds) {
  ...
  s = spring->s;
 #pragma omp parallel for ordered schedule(dynamic) private(j)
  for(j=0; j<numEqns; ++j) {
    #pragma omp ordered  
    q[j] = spring->q[j];
  }     
...
void springRightHandSide( struct SpringODE *spring, double *q, 
  ...
  //  dependent variables.
  #pragma omp parallel for ordered schedule(dynamic) private(i)
  for(i=0; i<2; ++i) {
    #pragma omp ordered
